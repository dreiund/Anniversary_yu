...
1. 生理期备注保存：样例代码的 `.onSubmit` 在 `axis: .vertical` 的 TextField 上不触发（回车=换行）——改为 `.onChange(of: note)`（iOS17 双参形式）即时保存，删除 `.onSubmit`。spec「备注（可选）+ 点选即存」语义为准。
2. 「这天经期结束」ghost 按钮：删除无效的 `.tint(DS.roseCycle)`（GhostPillButtonStyle 前景写死行动蓝，且全局唯一行动色本就是蓝）与 `.frame(maxWidth: .infinity)`（胶囊不随之撑满，徒增不一致）。
